...
The aim of this project is to gather, transform and load data into a database.
...
- We chose a csv file from kaggle.com (https://www.kaggle.com/noaa/deep-sea-corals) that consisted of records of more than 500,000 deep sea corals' locations from the National Oceanic and Atmospheric Administration (NOAA: https://deepseacoraldata.noaa.gov/).
...
- We first loaded the csv file as a dataframe on a jupyter notebook using Pandas. We selected the columns that were of interest to us and deleted the rows with NA values.
...
- Most of the species taxonomy is composed of 8 ranks: Kingdom, Phyllum, Class, Subclass, Family, Order, Genus and Species. But for some of the species there was an extra category, which wasn't always the same (sometimes it was a Suborder, or Subspecies, etc). We chose the species that had only the 8 ranks, to simplify the analysis. Even though we eliminated some records, our dataframe comprised more than 200,000 rows.
...
- On the coral information table, we imported the csv file we created that contained the original NOAA data, plus the AphiaID for each sp.
...
- We also added an info box for each marker that contains the information for that species in that locality: name of the species, latitude and longitude, depth at which it was found, name of the locality, and name and picture of the vernacular category to which it belongs:
